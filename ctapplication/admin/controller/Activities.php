...
class Activities extends \think\Controller
{
    ...
    public function onedollar($page=1)
    {
      $res=Db::table("ceb_Product")
          ->page($page,20)
          ->alias('p')
          ->field('P.ID,P.Title,P.Price,P.SaleNum,P.ClickNum,P.Quantity,P.TimeFd1,P.Status,P.Img,p.StartTime,s.Name as sname')
          ->join('ceb_ShopMain s','p.ShopID=s.ID')
          ->where("ActivityType='1' and Type='1'")
    ...
    public function onedollar_status_edit()
    {
      if (input('')) {
        ...
        $one=Db::table("ceb_Product")
            ...
            ->find();
        // dump($one);       
        welog($text=$status.'(一元购) —— '.$one['Title']);
        if ($res!=0) {
    ...
    public function onedollar_add($page=1){
      if (input('goodsname')) {
        ...
        $res=Db::table("ceb_Product")
            ->where($wh)
            ->update(['ActivityType' => '1','StartTime' => "$StartTime",'EndTime' => "$EndTime",'Quantity' => "$Quantity",'Leastnum' => "$Leastnum",'Mostnum' => "$Mostnum"]);
        if ($res!=0) {
          welog($text='添加一元购商品'.'——'.$one['Title']);  
          $this->success('添加成功',url('admin/Activities/onedollar'));
    ...
    public function delete()
    {
      if (input('')) {
        ...
        if ($res!=0) {
          welog($text='删除一元购商品'.'——'.$one['Title']); 
          $this->success('删除成功',url('admin/Activities/onedollar'));
        }else{
           $this->error('删除失败');
        }  
        // $a = array("msg"=>"hello"); 
    ...
    public function delete_all()
    {
      if (input('')) {
       ...
       $res=Db::table("ceb_Product")
            ...
            ->update(['ActivityType' => '0','StartTime' => "",'EndTime' => "",'Quantity' => "",'Leastnum' => "",'Mostnum' => ""]);
        if($res){
            welog($text='批量删除删除(一元购)商品');
            $this->success('批量删除商品成功',url('admin/Activities/onedollar'));
        }else{
            $this->error('批量删除商品失败');
        }
      }
    ...
    public function onedollar_shop($page=1){
      ...
      if (input('keyword')!='') {
        ...
        $hot = $keyword;
          ...
          ->field('P.ID,P.Title,P.Price,P.SaleNum,P.ClickNum,P.Quantity,P.TimeFd1,P.Status,P.Img,p.StartTime,s.Name as sname')
          ->join('ceb_ShopMain s','p.ShopID=s.ID')
          ->where("ActivityType='1' and Type='1' and s.Name like '%". $hot."%'")
          ->order('StartTime desc')
          ->select();
    ...
    public function onedollar_list($page=1){
      ...
      if (input('keyword')!='') {
        $hot = $keyword;
          ...
          ->field('P.ID,P.Title,P.Price,P.SaleNum,P.ClickNum,P.Quantity,P.TimeFd1,P.Status,P.Img,p.StartTime,s.Name as sname')
          ->join('ceb_ShopMain s','p.ShopID=s.ID')
          ->where("ActivityType='1' and Type='1' and p.Title like '%". $hot."%'")
          ->order('StartTime desc')
          ->select();
    ...
    public function edit(){
      ...
      $res=Db::table("ceb_Product")
        ...
        if ($res!=0) {
          welog($text='修改一元购商品'.'——'.$one['Title']); 
          $this->success('修改成功',url('admin/Activities/onedollar'));
        }else{
          $this->error('修改失败');
        }  
