...
class User extends \think\Controller
{
    ...
    public function user_list($page=1){
      if (input('keyword')!='') {
        ...
        $res=Db::table('ceb_Member')
            ->page($page,20)
            ->where("MemberType=2 and EnrolName='$hot' or NickName='$hot' or TrueName='$hot'")
            ->select();
        foreach ($res as $k => $v) {
    ...
    public function user_examine($page=1){
      ...
      if (input('keyword')!='') {
        // dump(input('keyword'));
        // $hot = iconv("utf-8","gbk","$keyword");cDecFd2
        $hot = $keyword;
        $res=DB::table('ceb_ApproveInfo')
            ...
            ->where("a.type=1 and a.Contact like '%". $hot."%' or a.Name like '%". $hot."%' or m.EnrolName like '%". $hot."%'")
            ->order('ID desc')
            ->select();
      }else{
        $res=DB::table('ceb_ApproveInfo')
    ...
    public function user_state($page=1){
      ...
      if (input('keyword')!='') {
        ...
        $res=DB::table('ceb_ApproveInfo')
            ...
            ->join('ceb_Member m','a.MemberID=m.ID')
            ->join('ceb_ShopMain s','s.MemberID=m.ID')
            ->where("a.type=1 and a.AuthentResult='$hot'")
            ->order('ID desc')
            ->select();
      }else{
        $res=DB::table('ceb_ApproveInfo')
