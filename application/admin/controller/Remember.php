...
class Remember extends \think\Controller
{
    ...
    { 
      ...
      if (input('keyword')||input('start')||input('end')) {
        ...
        if($start!='')
        {
            ...
            $where=array('a.CreateTime'=>array("egt",$start));
        }

        if($end!='')
        {
            // $where["a.CreateTime"]=array("elt",$end);
            $where=array('a.CreateTime'=>array("elt",$start));           
        }
    ...
    public function user_time($page=1){
      ...
      if ($start_time!='' && $end_time=='') {
        ...
        $res =Db::table('ceb_Member')->page($page,20)->where($wh)->select();
        ...
        $res =Db::table('ceb_Member')->page($page,20)->where('MemberType=0')->order('CreateTime desc')->select();
        $tem=Db::table('ceb_Member')->where($wh)->select();    
        $member_num=count($tem);
