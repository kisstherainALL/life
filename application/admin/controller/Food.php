...
class Food extends \think\Controller
{
    ...
    public function food_list($page=1){
      ...
      //实例化数据模型
      $data=new \app\admin\model\Food;
      $one=$data->index($page);
      $sum=$data->sum($page);
      $sales_sum=$data->sales($page);
      $tem_page=$sum/20;
      $lastpage=ceil($tem_page);

      $this->assign('res2',$one);
    ...
    public function info()
    {
      ...
      foreach ($data as $k => $v) {
        ...
        $where_pid=$wh['p.ID'];
        $dd=Db::table('ceb_DishesDetails')
           ->where('ProID',$where_pid)
           ->find();
        $more_img = explode('|',$dd['ShufflingImg']);
      // dump($more_img);
